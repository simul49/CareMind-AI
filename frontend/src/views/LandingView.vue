...
const features = [
  { badge: "MED", title: "Medicine Reminders", desc: "Never miss a dose. One-tap confirm, gentle reminders." },
  { badge: "BP", title: "Health Timeline", desc: "BP, heart rate, sleep & activity — all in one simple timeline." },
  { badge: "AI", title: "CareMind AI", desc: "A kind companion that understands your health context." },
  { badge: "SOS", title: "SOS Emergency", desc: "One big button. Family alerted, nearby hospital found." },
  { badge: "FAM", title: "Family Circle", desc: "Moments, chat and peace of mind for the whole family." },
  { badge: "DOC", title: "Doctor Connect", desc: "Your doctor sees the real picture — with your consent." },
];
</script>
    ...
    <div class="mx-auto max-w-5xl px-5 py-10 sm:py-16">
      ...
      <section class="mt-14 text-center sm:mt-20">
        <h1 class="mx-auto max-w-3xl text-4xl font-extrabold leading-tight sm:text-6xl">
          Smart Care.
          <span class="text-teal">Safe Living.</span><br />
          Peace of Mind.
        </h1>
        <p class="mx-auto mt-6 max-w-2xl text-xl text-ink/70">
          A private AI-powered care ecosystem that connects older adults, their family,
          their trusted doctor — and a caring AI companion.
        </p>
        ...
        <div class="mt-10 rounded-3xl bg-white/70 p-4 shadow-soft backdrop-blur">
          <p class="label mb-3">Hackathon demo — try a seeded account</p>
      ...
      <section class="mt-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="f in features" :key="f.title" class="card transition hover:-translate-y-1 hover:shadow-glow">
          <div class="grid h-14 w-14 place-items-center rounded-2xl bg-teal-light text-sm font-extrabold text-teal-dark">{{ f.badge }}</div>
          <h3 class="mt-3 text-xl font-extrabold">{{ f.title }}</h3>
          <p class="mt-1 text-ink/70">{{ f.desc }}</p>
        </div>
      </section>

      <footer class="mt-16 pb-6 text-center text-sm text-ink/50">
        CareMind AI — built for the 3-day hackathon. The AI companion never replaces a doctor.
      </footer>
    </div>
  </div>
</template>
